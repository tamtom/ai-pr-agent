...
- **Review**:       `python -m ai_pr_agent.cli --pr_url=<pr_url>  review`
- **Describe**:     `python -m ai_pr_agent.cli --pr_url=<pr_url>  describe`
- **Improve**:      `python -m ai_pr_agent.cli --pr_url=<pr_url>  improve`
- **Ask**:          `python -m ai_pr_agent.cli --pr_url=<pr_url>  ask "Write me a poem about this PR"`
- **Reflect**:      `python -m ai_pr_agent.cli --pr_url=<pr_url>  reflect`
- **Update Changelog**:      `python -m ai_pr_agent.cli --pr_url=<pr_url>  update_changelog`
...
```
python -m ai_pr_agent.cli --pr_url=<pr_url>  /review --pr_reviewer.extra_instructions="focus on the file: ..."
```
...
```
Any configuration value in [configuration file](https://github.com/Codium-ai/pr-agent/blob/main/ai_pr_agent/settings/configuration.toml) file can be similarly edited. Comment `/config` to see the list of available configurations.
...
The [github_app](https://github.com/Codium-ai/pr-agent/blob/main/ai_pr_agent/settings/configuration.toml#L108) section defines GitHub app specific configurations.  
...
For example, if your local `.ai_pr_agent.toml` file contains:
```
[pr_description]
...
You can configure settings for `GitHub Action` by adding environment variables under the env section in `.github/workflows/ai_pr_agent.yml` file. 
...
The JSON structure is equivalent to the yaml data structure defined in [pr_reviewer_prompts.toml](https://github.com/idubnori/pr-agent/blob/main/ai_pr_agent/settings/pr_reviewer_prompts.toml).

Note that you can give additional config parameters by adding environment variables to `.github/workflows/ai_pr_agent.yml`, or by using a `.ai_pr_agent.toml` [configuration file](https://pr-agent-docs.codium.ai/usage-guide/configuration_options/#global-configuration-file) in the root of your repo

For example, you can set an environment variable: `pr_description.publish_labels=false`, or add a `.ai_pr_agent.toml` file with the following content:
```
[pr_description]
...
## BitBucket App
Similar to GitHub app, when running PR-Agent from BitBucket App, the default [configuration file](https://github.com/Codium-ai/pr-agent/blob/main/ai_pr_agent/settings/configuration.toml) from a pre-built docker will be initially loaded.
