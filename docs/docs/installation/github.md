...
1) Add the following file to your repository under `.github/workflows/ai_pr_agent.yml`:
...
4) You may configure PR-Agent by adding environment variables under the env section corresponding to any configurable property in the [configuration](https://github.com/Codium-ai/pr-agent/blob/main/ai_pr_agent/settings/configuration.toml) file. Some examples:
```yaml
      env:
...
```
cp ai_pr_agent/settings/.secrets_template.toml ai_pr_agent/settings/.secrets.toml
...
```
   ...
   - Set deployment_type to 'app' in [configuration.toml](https://github.com/Codium-ai/pr-agent/blob/main/ai_pr_agent/settings/configuration.toml)
    ...
    // ...
              volumeMounts:
                - mountPath: /app/ai_pr_agent/settings_prod
                  name: settings-volume
    ```
...
> However, you can override the default tool parameters by uploading a local configuration file `.ai_pr_agent.toml`
...
5. Set the `git_provider` value to `codecommit` in the `ai_pr_agent/settings/configuration.toml` settings file
6. Set the `PYTHONPATH` to include your `pr-agent` project directory
    ...
    * Option B: Set `PYTHONPATH` and run the CLI in one command, for example:
        * `PYTHONPATH="/PATH/TO/PROJECTS/pr-agent python ai_pr_agent/cli.py [--ARGS]`
...
```sh
PYTHONPATH="/PATH/TO/PROJECTS/pr-agent" python ai_pr_agent/cli.py \
