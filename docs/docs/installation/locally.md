...
For other git providers, update CONFIG.GIT_PROVIDER accordingly, and check the `ai_pr_agent/settings/.secrets_template.toml` file for the environment variables expected names and values.
...
chmod 600 ai_pr_agent/settings/.secrets.toml
...
```
python3 -m ai_pr_agent.cli --pr_url <pr_url> review
python3 -m ai_pr_agent.cli --pr_url <pr_url> ask <your question>
python3 -m ai_pr_agent.cli --pr_url <pr_url> describe
python3 -m ai_pr_agent.cli --pr_url <pr_url> improve
python3 -m ai_pr_agent.cli --pr_url <pr_url> add_docs
python3 -m ai_pr_agent.cli --pr_url <pr_url> generate_labels
python3 -m ai_pr_agent.cli --issue_url <issue_url> similar_issue
...
```

[Optional] Add the ai_pr_agent folder to your PYTHONPATH
```
export PYTHONPATH=$PYTHONPATH:<PATH to ai_pr_agent folder>
```