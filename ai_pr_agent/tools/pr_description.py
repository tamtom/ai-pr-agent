...
from ai_pr_agent.algo.pr_processing import get_pr_diff, retry_with_fallback_models
from ai_pr_agent.algo.token_handler import TokenHandler
from ai_pr_agent.algo.utils import load_yaml, set_custom_labels, get_user_labels, ModelType, show_relevant_configurations
...
class PRDescription:
    ...
    async def _prepare_prediction(self, model: str) -> None:
        if get_settings().pr_description.use_description_markers and 'ai_pr_agent:' not in self.user_description:
            return None
    ...
    def _prepare_pr_answer_with_markers(self) -> Tuple[str, str, str, List[dict]]:
        ...
        if ai_type and not re.search(r'<!--\s*ai_pr_agent:type\s*-->', body):
            pr_type = f"{ai_header}{ai_type}"
            body = body.replace('ai_pr_agent:type', pr_type)

        ai_summary = self.data.get('description')
        if ai_summary and not re.search(r'<!--\s*ai_pr_agent:summary\s*-->', body):
            summary = f"{ai_header}{ai_summary}"
            body = body.replace('ai_pr_agent:summary', summary)

        ai_walkthrough = self.data.get('pr_files')
        walkthrough_gfm = ""
        pr_file_changes = []
        if ai_walkthrough and not re.search(r'<!--\s*ai_pr_agent:walkthrough\s*-->', body):
            try:
                walkthrough_gfm, pr_file_changes = self.process_pr_files_prediction(walkthrough_gfm,
                                                                                    self.file_label_dict)
                body = body.replace('ai_pr_agent:walkthrough', walkthrough_gfm)
            except Exception as e:
                get_logger().error(f"Failing to process walkthrough {self.pr_id}: {e}")
                body = body.replace('ai_pr_agent:walkthrough', "")
