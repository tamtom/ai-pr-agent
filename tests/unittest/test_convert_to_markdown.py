...
from ai_pr_agent.algo.utils import convert_to_markdown
...
class TestConvertToMarkdown:
    ...
    def test_simple_dictionary_input(self):
        input_data = {'review': {
            ...
            {'relevant_file': '``ai_pr_agent/git_providers/git_provider.py\n``', 'language': 'python\n',
...
class TestBR:
    def test_br1(self):
        file_change_description = '- Imported `FilePatchInfo` and `EDIT_TYPE` from `ai_pr_agent.algo.types` instead of `ai_pr_agent.git_providers.git_provider`.'
        ...
        expected_output = ('<li>Imported <code>FilePatchInfo</code> and <code>EDIT_TYPE</code> from '
                           '<code>ai_pr_agent.algo.types</code> instead <br>of '
                           '<code>ai_pr_agent.git_providers.git_provider</code>.')
